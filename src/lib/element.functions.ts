...
export const getElements = createServerFn({ method: "GET" })
  .validator((input: unknown) => getElementsSchema.parse(input))
  .handler(async ({ data }) => {
    ...
    const technologies = await db
      ...
      .where(
        sql`${elementTechnology.elementId} IN ${elementIds}`,
      );
    ...
    const links = await db
      ...
      .from(elementLink)
      .where(
        sql`${elementLink.elementId} IN ${elementIds}`,
      );
...
export const createElement = createServerFn({ method: "POST" })
  .validator((input: unknown) => createElementSchema.parse(input))
  .handler(async ({ data }) => {
...
export const updateElement = createServerFn({ method: "POST" })
  .validator((input: unknown) => updateElementSchema.parse(input))
  .handler(async ({ data }) => {
    const { session, memberRole } = await getSessionAndOrg();
    assertRole(memberRole, ["owner", "admin", "editor"]);
...
export const deleteElement = createServerFn({ method: "POST" })
  .validator((input: unknown) => deleteElementSchema.parse(input))
  .handler(async ({ data }) => {
    const { session, memberRole } = await getSessionAndOrg();
    ...
    await db.execute(sql`
      WITH RECURSIVE descendants AS (
        SELECT id FROM element WHERE id = ${data.id}
        UNION ALL
        SELECT e.id FROM element e
        JOIN descendants d ON e.parent_element_id = d.id
        WHERE e.deleted_at IS NULL
      )
      UPDATE element SET deleted_at = NOW(), updated_by = ${session.user.id}
...
export const addTechnology = createServerFn({ method: "POST" })
  .validator((input: unknown) => addTechnologySchema.parse(input))
  .handler(async ({ data }) => {
    const { memberRole } = await getSessionAndOrg();
    assertRole(memberRole, ["owner", "admin", "editor"]);
...
export const updateTechnology = createServerFn({ method: "POST" })
  .validator((input: unknown) => updateTechnologySchema.parse(input))
  .handler(async ({ data }) => {
    const { memberRole } = await getSessionAndOrg();
    assertRole(memberRole, ["owner", "admin", "editor"]);
...
export const removeTechnology = createServerFn({ method: "POST" })
  .validator((input: unknown) => removeTechnologySchema.parse(input))
  .handler(async ({ data }) => {
    const { memberRole } = await getSessionAndOrg();
    assertRole(memberRole, ["owner", "admin", "editor"]);
...
export const reorderTechnologies = createServerFn({ method: "POST" })
  .validator((input: unknown) => reorderTechnologiesSchema.parse(input))
  .handler(async ({ data }) => {
    const { memberRole } = await getSessionAndOrg();
    assertRole(memberRole, ["owner", "admin", "editor"]);
...
export const addLink = createServerFn({ method: "POST" })
  .validator((input: unknown) => addLinkSchema.parse(input))
  .handler(async ({ data }) => {
    const { memberRole } = await getSessionAndOrg();
    assertRole(memberRole, ["owner", "admin", "editor"]);
...
export const updateLink = createServerFn({ method: "POST" })
  .validator((input: unknown) => updateLinkSchema.parse(input))
  .handler(async ({ data }) => {
    const { memberRole } = await getSessionAndOrg();
    assertRole(memberRole, ["owner", "admin", "editor"]);
...
export const removeLink = createServerFn({ method: "POST" })
  .validator((input: unknown) => removeLinkSchema.parse(input))
  .handler(async ({ data }) => {
    const { memberRole } = await getSessionAndOrg();
    assertRole(memberRole, ["owner", "admin", "editor"]);
